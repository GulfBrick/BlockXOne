...
import { applicationDetailsSchema, type EvidenceDocument, type LegacyApplicationDetails, type PortalApplication, type PortalSnapshot, type WealthManagerApplicationDetailsV2 } from '@/lib/portal/contracts'
...
const documents: EvidenceDocument[] = ['IDENTITY', 'COMPANY', 'BENEFICIAL_OWNERS'].map((kind, index) => ({ id: `${index + 4}4444444-4444-4444-8444-444444444444`, kind, title: `Fictional ${kind} evidence`, storage_path: `${entryActorId}/test-${index}.pdf`, sha256: 'a'.repeat(64), size: 100, mime_type: 'application/pdf' }))
const investor: LegacyApplicationDetails = { full_name: 'Alex Example', country: 'ZA', investor_type: 'INDIVIDUAL', company_name: '', registration_reference: '', source_of_funds: 'Fictional savings from employment income.', beneficial_owners: '', experience: 'Fictional long-term investment objectives.', documents: [documents[0]], test_data_acknowledged: true }
const manager: WealthManagerApplicationDetailsV2 = { details_version: 2, full_name: 'Alex Representative', country: 'ZA', company_name: 'Example Advisory Test', registration_reference: 'SYNTHETIC-001', beneficial_owners: 'Fictional sole owner Alex Example has 100 percent.', business_activities: 'Fictional wealth advisory requesting fund-structuring services.', representative_position: 'Appointed representative', authority_basis: 'Fictional board mandate authorises an organisation application only.', documents, test_data_acknowledged: true }
