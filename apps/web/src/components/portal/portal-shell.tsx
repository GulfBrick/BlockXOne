...
export function PortalShell({ user, organisationName, capabilities, active, title, description, eyebrow, breadcrumbs = [], actions, children, release, onboardingAvailable = true }: {
  user: { id: string; email: string }; organisationName?: string; capabilities: PortalNavigationCapabilities; active: PortalNavigationKey;
  title: string; description?: string; eyebrow?: string; breadcrumbs?: { label: string; href?: string }[]; actions?: ReactNode; children: ReactNode; release?: PlatformRelease; onboardingAvailable?: boolean;
}) {
  ...
  const navigation = portalNavigation(capabilities, onboardingAvailable)
  ...
  return <div className={styles.portal}>
    ...
    <aside className={styles.sidebar}>
      <Link href="/portal" className={styles.brand} aria-label="BlockXOne portal home"><Image src="/brand/blockxone-lockup-horizontal.png" alt="BlockXOne" width={236} height={48} className={styles.brandArtwork} priority /></Link>
