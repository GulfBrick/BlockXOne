  ...
  return <div className={styles.stack}>
    ...
    <Panel title={product ? 'Edit product draft' : 'Create a product'} description="Prepare fund or real-estate terms. Submission seals an immutable offering package for separate decisions.">
      ...
      <form className={styles.form} onSubmit={async event => { event.preventDefault(); if (!acknowledged) return; const saved = await command.submit(product ? 'save_product' : 'create_product', product ? { product_id: product.id, expected_revision: product.revision, terms } : { organisation_id: organisationId, terms }); if (saved && !product) router.push(portalScopeHref('/portal/products', operatingContext)) }}>
        ...
        <div className={styles.formFoot}><p>{product ? `Saving uses workflow revision ${product.revision}; stale edits will be rejected. This counter is not the offering package number.` : 'The draft will be scoped to the selected organisation. No reviewer authority is assigned by this form.'}</p><button type="submit" className={styles.button} disabled={locked || !acknowledged}>{product ? 'Save revised draft' : 'Create product draft'}</button></div>
      </form>
    </Panel>
  </div>
}
