...
const markerKey = (actorId: string) => `bx1-portal:fegnnnlseuejkrusbbkv:${actorId}:pending-request`
const PortalCommandContext = createContext<{ actorId: string; requests: { key: string; command: string }[] }>({ actorId: '', requests: [] })
export function PortalCommandProvider({ snapshot, children }: { snapshot: PortalSnapshot; children: ReactNode }) {
  return <PortalCommandContext.Provider value={{ actorId: snapshot.actor.id, requests: snapshot.requests ?? [] }}>{children}</PortalCommandContext.Provider>
}
...
export function reconcilePortalMarker(storage: MarkerStorage, actorId: string, requests: { key: string; command: string }[]): Marker | null {
  const marker = readMarker(storage.getItem(markerKey(actorId)))
  if (marker && requests.some(request => request.key === marker.key && request.command === marker.command)) { storage.removeItem(markerKey(actorId)); return null }
  return marker
}
export async function prepareDurablePortalCommand(storage: MarkerStorage, actorId: string, request: PortalCommand, requests: { key: string; command: string }[]): Promise<PortalCommand> {
  if (!actorId) throw new Error('Your authenticated account is unavailable. Reload before continuing.')
  const marker = reconcilePortalMarker(storage, actorId, requests)
  const original = storage.getItem(markerKey(actorId))
  const digest = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(JSON.stringify({ command: request.command, payload: request.payload })))
  ...
  if (storage.getItem(markerKey(actorId)) !== original) throw new Error('Another request was recorded while this action was preparing. Refresh before continuing.')
  ...
  storage.setItem(markerKey(actorId), JSON.stringify({ key: prepared.key, command: prepared.command, payloadHash }))
  return prepared
}
export function clearPortalMarker(storage: MarkerStorage, actorId: string, request: PortalCommand) {
  const marker = readMarker(storage.getItem(markerKey(actorId)))
  if (marker?.key === request.key && marker.command === request.command) storage.removeItem(markerKey(actorId))
}

export async function postPortalCommand(command: PortalCommand): Promise<PortalSnapshot> {
  const response = await fetch('/api/portal/command', { method: 'POST', credentials: 'same-origin', cache: 'no-store', redirect: 'error', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(command), signal: AbortSignal.timeout(45000) })
...
export function usePortalCommand(onSaved: (snapshot: PortalSnapshot) => void) {
  ...
  useEffect(() => {
    ...
    try {
      const marker = reconcilePortalMarker(sessionStorage, context.actorId, context.requests)
  ...
  }, [context.actorId, context.requests])
  async function dispatch(request: PortalCommand) {
    if (lock.current) return false
    lock.current = true; setBusy(true); setMessage('')
    let sent = false
    try {
      const prepared = await prepareDurablePortalCommand(sessionStorage, context.actorId, request, context.requests)
      savedRequest.current = prepared
      sent = true
      const snapshot = await postPortalCommand(prepared)
      clearPortalMarker(sessionStorage, context.actorId, prepared)
      ...
      if (definitive && savedRequest.current) { clearPortalMarker(sessionStorage, context.actorId, savedRequest.current); savedRequest.current = null }
      setUnknown(sent && !definitive)
