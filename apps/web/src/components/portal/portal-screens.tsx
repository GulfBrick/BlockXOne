...
export function SubscriptionOrders({ subscriptions, products, incoming, onSaved, operatingContext }: {
    ...
    {subscriptions.length ? <div className={styles.tableWrap}><table className={styles.table}><thead><tr><th scope="col">Product</th><th scope="col">Order reference</th><th scope="col">Investment account</th><th scope="col">Units</th><th scope="col">Instruction value</th><th scope="col">Status</th><th scope="col">Next action</th></tr></thead><tbody>
      {subscriptions.map(subscription => {
        ...
        return <tr key={subscription.id}><td><strong>{subscription.product_name}</strong>{product ? <small>{product.terms.asset_type === 'FUND' ? 'Fund' : 'Real estate'}</small> : null}</td>
          ...
          <td>{subscription.units}</td><td>{money(subscription.amount_minor)}</td><td><StatusBadge status={subscription.status} /></td>
          <td>{incoming ? <div className={styles.stack}>{product ? <Link href={portalScopeHref('/portal/products/detail', operatingContext, product.id)}>Open product and order book</Link> : null}<span className={styles.muted}>{subscription.status === 'AWAITING_FUNDING' ? 'Funding verification is not connected. This order is not settled or issued.' : 'Reservation cancelled; no funding or issuance is inferred.'}</span></div>
            : subscription.status === 'AWAITING_FUNDING' ? <SubscriptionCancel id={subscription.id} onSaved={onSaved} /> : <span className={styles.muted}>No pending action</span>}</td>
        </tr>
      })}
...
function OperatingPortalScreen({ data, view, id, operatingContext, release, scope, scopes }: PortalScreenProps) {
  ...
  const roleTitle = role === 'Investor' ? 'Your investments and orders.' : role === 'OfferingManager' || role === 'IssuerFundManager' ? 'Your products and incoming orders.' : role === 'ComplianceOfficer' ? 'Your compliance work queue.' : undefined
  ...
  const breadcrumbs = view === '/portal' ? [] : [{ label: config.active === 'products' ? 'Products & offerings' : config.active === 'compliance' ? 'Compliance' : config.active === 'opportunities' ? 'Opportunities' : config.active === 'portfolio' ? 'My orders' : 'My onboarding', href: view.includes('/detail') || view.endsWith('/new') ? href(parentPath) : undefined }, ...(view.includes('/detail') ? [{ label: selected?.terms.name || selectedApplication?.details.full_name || 'Details' }] : view.endsWith('/new') ? [{ label: 'New product' }] : [])]
  ...
  function content() {
    ...
    if (view === '/portal/portfolio') return canInvest ? <div className={styles.stack}>{orderStats()}<Notice title="Reservations are not holdings">These are subscription instructions and funding obligations. No token holding or performance balance is manufactured from a reservation.</Notice><SubscriptionOrders subscriptions={ownSubscriptions} products={snapshot.products} incoming={false} onSaved={setSnapshot} operatingContext={operatingContext} /></div> : unavailable()
