...
import { isWealthManagerDetailsV2, representativeMandateNextOwnerLabel, type PortalCapability, type PortalOrganisation, type PortalOrganisationMandate, type PortalPageData, type PortalPath, type PortalProduct, type PortalSnapshot, type PortalSubscription } from '@/lib/portal/contracts'
...
function Opportunities({ products, snapshot, operatingContext }: { products: PortalProduct[]; snapshot: PortalSnapshot; operatingContext?: PortalOperatingContext }) {
  return <div className={styles.stack}>
    <Notice title="Read before you subscribe">Published offerings matching your approved admission country and investor classification are shown for review. An active investment account and a current product-specific eligibility decision are required before subscribing. No returns or liquidity are guaranteed.</Notice>
...
function ComplianceQueue({ snapshot, operatingContext }: { snapshot: PortalSnapshot; operatingContext?: PortalOperatingContext }) {
  ...
  return <div className={styles.stack}>
    <div className={styles.statGrid}><Statistic label="Client applications" value={applications.length} hint="Submitted evidence packages in this review scope" /><Statistic label="Offering reviews" value={products.length} hint="Exact product revisions awaiting a decision" /><Statistic label="Product eligibility actions" value={eligibilityUnavailable ? 'Unavailable' : eligibilityCases.length} hint="Submitted cases and revocable approvals in this review scope" /><Statistic label="Representative mandates" value={snapshot.mandate_queue_available === true && Array.isArray(snapshot.organisation_mandates) ? scopedMandates(snapshot, operatingContext).filter(item => item.can_review || item.can_revoke).length : 'Unavailable'} hint="Independent appointment cases in this exact scope" /><Statistic label="Review provider" value="Manual" hint="Rehearsal evidence review · no live KYC-provider claim" /></div>
...
function OperatingPortalScreen({ data, view, id, operatingContext, release, scope, scopes }: PortalScreenProps) {
  ...
  const canInvest = role === 'Investor' || (operatingContext?.mode !== 'ROLE' && hasInvestorCapacity)
  ...
  const breadcrumbs = view === '/portal' ? [] : view === '/portal/orders/detail' ? [{ label: 'My operating workspace', href: href('/portal') }, { label: 'Subscription funding' }] : [{ label: config.active === 'products' ? 'Products & offerings' : config.active === 'compliance' ? 'Compliance' : config.active === 'opportunities' ? 'Opportunities' : config.active === 'portfolio' ? 'My orders' : 'My onboarding', href: view.includes('/detail') || view.endsWith('/new') ? href(parentPath) : undefined }, ...(view.includes('/detail') ? [{ label: selected?.terms.name || selectedApplication?.details.full_name || (selectedMandate ? 'Representative appointment' : selectedEligibility ? 'Product eligibility case' : 'Details') }] : view.endsWith('/new') ? [{ label: 'New product' }] : [])]
  ...
  function content() {
    ...
    if (view === '/portal/compliance') return canReview ? <ComplianceQueue snapshot={reviewSnapshot} operatingContext={operatingContext} /> : role === 'SuperAdmin' ? <RepresentativeMandateQueue snapshot={snapshot} operatingContext={operatingContext} /> : unavailable()
    ...
    if (view === '/portal/opportunities/detail') return canInvest && selected?.status === 'PUBLISHED' ? <div className={styles.wideGrid}><div className={styles.stack}><Notice title="Fictional test offering">{selected.terms.summary}</Notice><ProductFacts product={selected} /><ProductNarrative product={selected} /><OfferingDocuments product={selected} /></div><div className={styles.stack}><ProductEligibilityPanel product={selected} snapshot={snapshot} onSaved={setSnapshot} operatingContext={operatingContext} /><SubscriptionForm product={selected} snapshot={snapshot} onSaved={setSnapshot} operatingContext={operatingContext} /></div></div> : unavailable()
    if (view === '/portal/portfolio') return canInvest ? <div className={styles.stack}>{orderStats()}<InvestmentAccountPanel snapshot={snapshot} onSaved={setSnapshot} operatingContext={operatingContext} /><Notice title="Reservations are not holdings">These are subscription instructions with reserved units. A reservation does not confirm funding, token ownership or investment performance.</Notice><SubscriptionOrders subscriptions={ownSubscriptions} products={snapshot.products} incoming={false} onSaved={setSnapshot} operatingContext={operatingContext} funding={snapshot.funding} /></div> : unavailable()
    ...
    if (role === 'SuperAdmin' && operatingContext?.mode === 'ROLE') return <div className={styles.stack}><RepresentativeMandateQueue snapshot={snapshot} operatingContext={operatingContext} /><Panel title="Separate gated administration service" description="The representative-mandate queue above is the connected appointment handoff. The broader administration service requires its own configured authority root and may be unavailable in this environment; it is not needed to apply a reviewed mandate here. Business and wallet-signing powers remain separate."><div className={styles.actions}><Link href={`/workspace/administration?organisation=${encodeURIComponent(operatingContext.organisationId)}`} className={styles.buttonSecondary}>Open gated administration, if configured</Link><a href="/workspace/security" className={styles.buttonSecondary}>Account security</a></div></Panel></div>
