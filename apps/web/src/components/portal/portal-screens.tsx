...
function OperatingPortalScreen({ data, view, id, operatingContext, release, scope, scopes }: PortalScreenProps) {
  ...
  const breadcrumbs = view === '/portal' ? [] : view === '/portal/orders/detail' ? [{ label: 'My operating workspace', href: href('/portal') }, { label: 'Subscription funding' }] : [{ label: config.active === 'products' ? 'Products & offerings' : config.active === 'compliance' ? 'Compliance' : config.active === 'opportunities' ? 'Opportunities' : config.active === 'portfolio' ? 'My orders' : 'My onboarding', href: view.includes('/detail') || view.endsWith('/new') ? href(parentPath) : undefined }, ...(view.includes('/detail') ? [{ label: selected?.terms.name || selectedApplication?.details.full_name || (selectedEligibility ? 'Product eligibility case' : 'Details') }] : view.endsWith('/new') ? [{ label: 'New product' }] : [])]
