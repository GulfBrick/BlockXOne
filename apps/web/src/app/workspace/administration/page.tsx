...
export default async function AdministrationPage({ searchParams }: Props) {
  ...
  return <PublicShell><main id="main-content" className="mx-auto w-full max-w-5xl px-4 py-12 sm:px-6 sm:py-16 lg:px-8">
    ...
    {(view.availability === 'ready' || view.availability === 'hold') && organisations.length ? <Link href={`/workspace/administration/staff-invitations?organisation=${encodeURIComponent(query?.organisationId ?? organisations[0].id)}`} className="mt-6 inline-flex min-h-11 items-center rounded-lg border border-bxo-accent-border px-4 text-sm font-semibold text-bxo-accent-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-bxo-accent-primary">Manage first-time staff invitations</Link> : null}
