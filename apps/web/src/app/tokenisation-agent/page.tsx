  ...
  const fetchStats = async () => {
    try {
      ...
      const [subsResp, redemptionsResp, whitelistResp, batchesResp] = await Promise.all([
        fetch(`${API_BASE}/v1/debug/subscriptions`, { headers: { 'Authorization': `Bearer ${token}` } }),
        fetch(`${API_BASE}/v1/redemptions?status=APPROVED`, { headers: { 'Authorization': `Bearer ${token}` } }),
        fetch(`${API_BASE}/v1/debug/whitelist-requests`, { headers: { 'Authorization': `Bearer ${token}` } }),
        fetch(`${API_BASE}/v1/token-batches`, { headers: { 'Authorization': `Bearer ${token}` } })
      ])
      ...
      const mintRequests = (subs as SubscriptionSummary[] | null | undefined || []).filter((s) =>
      ...
      const whitelistQueue = (whitelist as WhitelistRequestSummary[] | null | undefined || []).filter((w) =>
        w.status === 'PENDING' || w.status === 'APPROVED'
      ).length

      // Count active token batches (unique offerings with confirmed batches)
      const uniqueOfferings = new Set(
        (batches as TokenBatchSummary[] | null | undefined || [])
