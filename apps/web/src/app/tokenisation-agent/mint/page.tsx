...
export default function MintTokensPage() {
  ...
  const fetchSubscriptions = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8080/v1/debug/subscriptions', {
        headers: {
  ...
  const handleMint = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8080/v1/token-batches/mint', {
        method: 'POST',
        headers: {
      ...
      setError(err.message || 'Mint operation failed')
    } finally {
      setMinting(false)
    }
  }
