...
export async function loadRoleDashboard(query: DashboardQuery) {
  ...
  const applicant = query.mode === 'applicant' || !workspace
  ...
  if (applicant) {
    if (portal) return { kind: 'applicant' as const, portal, release, operatingContext, scopes: workspace ? dashboardScopes(workspace) : [] }
